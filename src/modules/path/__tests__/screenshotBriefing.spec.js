/**
 * Visual snapshot for issue #49 path node briefing sheet.
 * Run: SCREENSHOT=1 npm test -- --run src/modules/path/__tests__/screenshotBriefing.spec.js
 */
...
vi.mock("@/shared/api.js", () => ({
  ...
  getTeachingContent: vi.fn().mockResolvedValue({
    grammar_points: ["ser 和 estar 的基本区别", "定冠词 el/la 的用法"],
    words: [{ word: "hola" }, { word: "gracias" }, { word: "adiós" }, { word: "por favor" }],
...
describe.skipIf(!takeScreenshot)("screenshotBriefing", () => {
  beforeEach(() => {
  ...
  it("writes path briefing sheet screenshot", async () => {
    ...
    await flushPromises();
    await vi.waitUntil(() => document.body.querySelector(".briefing-sheet"), { timeout: 2000 });
    ...
    const outHtml = join(outDir, "issue49-path-briefing.html");
    const outPng = join(outDir, "issue49-path-briefing.png");
