...
function makeRouter() {
  return createRouter({
    ...
    routes: [
      ...
      { path: "/learn/vocab", name: "vocab", component: { template: "<div>vocab</div>" }, meta: { parent: "learn" } },
...
describe("AppShell bottom-nav safe-area", () => {
  ...
  it("androidBridge.js defines __amigaSetInsets that sets --safe-* CSS custom properties on <html>", () => {
    ...
    expect(bridge).toMatch(/__amigaSetInsets\s*=/);
    expect(bridge).toMatch(/--safe-top/);
    expect(bridge).toMatch(/--safe-bottom/);
    expect(bridge).toMatch(/--safe-left/);
    expect(bridge).toMatch(/--safe-right/);
  });
});
