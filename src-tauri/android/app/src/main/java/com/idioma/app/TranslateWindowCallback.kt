 ...
 * floating toolbar.
 *
 * Adds Amiga to the [ActionMode] menu that WebView and the OEM have already
 * created. It does not replace or restart the selection mode, so system actions
 * (Copy / Share / Select all / system Translate) keep their original callbacks
 * and vendor-specific toolbar positioning.
 *
...
class TranslateWindowCallback(private val webView: WebView) {
    ...
    /** Add Amiga to the existing selection menu without replacing the mode. */
