/**
 * Visual snapshot for learn hub lesson-article-match focus (#92).
 * Run: SCREENSHOT=1 npm test -- --run src/modules/learn/__tests__/screenshotLessonArticleHubFocus.spec.js
 */
...
function defaultInvoke(cmd) {
  ...
  if (cmd === "get_articles_cmd") {
    return Promise.resolve([
      {
        id: 11,
        original_title: "昨夜科技要闻",
        original_body: "La economía crece con hola y gracias en el banco.",
        rewritten_body: "La economía crece con hola y gracias en el banco.",
        hot_rank: 1,
      },
    ]);
  }
...
describe.skipIf(!takeScreenshot)("screenshotLessonArticleHubFocus", () => {
  beforeEach(() => {
  ...
  it("writes lesson-article-match focus hero screenshot", async () => {
    ...
    expect(wrapper.text()).toContain("在新闻里巩固刚学的词");
    expect(wrapper.text()).toContain("昨夜科技要闻");
    expect(wrapper.text()).toContain("去读这篇文章");
    writeScreenshot(wrapper, "issue92-lesson-article-match-focus");
  });
});