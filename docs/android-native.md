...
**首选 `run-android.bat`**，不是 `build-android.bat`。等价于：

```
npx tauri android dev [--device <name>] --exit-on-panic
```

Tauri 自动选择已连接设备/模拟器；可用环境变量 `AMIGA_ANDROID_DEVICE` 指定（见 `run-android.bat` 注释）。
