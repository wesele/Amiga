/**
 * Visual snapshot for learn hub comprehension-retake focus hero (#77).
 * Run: SCREENSHOT=1 npm test -- --run src/modules/learn/__tests__/screenshotComprehensionHubFocus.spec.js
 */
...
function defaultInvoke(cmd) {
  ...
  if (cmd === "get_articles_cmd") {
    return Promise.resolve([
      { id: 8, original_title: "昨夜科技要闻", hot_rank: 1 },
      { id: 9, original_title: "全球气候观察", hot_rank: 2 },
    ]);
  }
  if (cmd === "get_articles_reading_status_cmd") {
    return Promise.resolve([
      {
        article_id: 8,
        read_at: "2026-07-02 22:00:00",
        completed: true,
        comprehension_skipped: true,
      },
      {
        article_id: 9,
        read_at: "2026-07-02 21:00:00",
        completed: true,
...
describe.skipIf(!takeScreenshot)("screenshotComprehensionHubFocus", () => {
  beforeEach(() => {
  ...
  it("writes comprehension-retake focus hero screenshot", async () => {
    ...
    expect(wrapper.text()).toContain("补测《昨夜科技要闻》的理解");
    expect(wrapper.text()).toContain("上次跳过了快检");
    expect(wrapper.text()).toContain("还有 1 篇待补测");
    writeScreenshot(wrapper, "issue77-comprehension-retake-focus");
  });
});